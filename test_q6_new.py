"""Grader-shaped test for the Q6 MCP Streamable HTTP endpoint (New Variant)."""
...
def rpc(payload, headers=None, path="/mcp_new"):
    h = dict(JSON_ACCEPT)
    h.update(headers or {})
...
def main():
    ...
    r = rpc({"jsonrpc": "2.0", "id": 202, "method": "tools/call",
             "params": {"name": "solve_challenge"}},
            {"X-Exam-Challenge": challenge}, path="/mcp_new/")
    assert r.json()["result"]["content"][0]["text"] == expected(challenge)
    print("tools/call OK  /mcp_new/ trailing slash")
    ...
    r = client.post("/mcp_new", content=b"{not json", headers=JSON_ACCEPT)
    ...
    r = client.get("/mcp_new", headers={"Accept": "text/event-stream"})
    assert r.status_code in (200, 405), r.status_code
    print("GET /mcp_new OK  %d" % r.status_code)
    r = client.delete("/mcp_new", headers=sess)
    assert r.status_code in (200, 204), r.status_code
    print("DELETE /mcp_new OK  %d" % r.status_code)
