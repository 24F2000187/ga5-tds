"""Q5 (New Variant) - Agent Harness: Run Budget & Loop Guard.
...
string values normalized, and any field named ``trace_id`` or ``request_id`` dropped -- all
...
def canonicalize(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            name = str(key)
            if name.strip() in ("trace_id", "request_id"):
                continue
    ...
    if repeat >= _REPEAT_THRESHOLD:
        ...
        return {
            ...
            "reason": (
                ...
                "functionally identical args (ignoring key order, whitespace and trace_id)."
            ),
        }
...
@router.post("/q5_new/check")
async def check(body: dict) -> dict:
    return evaluate(body)


@router.post("/check_loop_new")
async def check_alias(body: dict) -> dict:
    return evaluate(body)
